...
In these steps, `<kit>` is the kit folder, named `{{KIT}}`: the folder that holds
`check_runbook.py`, `FORMAT.md` and this file (look for it at the root of the repository). Run
...
If you cannot ask the person (you run unattended, or they do not answer), do not guess a
threshold, a command or an approver. Leave the item to a person: cover it with an owner gate
whose `approve` says what must be decided, and list it under "open questions" in your report. A
default you choose yourself goes in a comment next to the value
(`# default chosen by the agent: ...`) and in the report.
