...
The sign-in page and the hub are plain pages with a code field and one tile per workspace. They support light and dark themes (stored under the same `alpaca.theme` key as the cockpit, otherwise following the device) and reduced motion. Their files are `alpaca/web/login.html`, `workspaces.html`, `vault.css` and `vault.js`, served publicly from `/login/assets/`.
